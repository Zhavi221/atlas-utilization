### Author: Christian Agrell (see arXiv:1901.03134)
### Code Source: https://github.com/cagrell/gp_constr/
...
class GPmodel():
    ...
    def __init__(self, kernel, likelihood = 1, mean = 0, noise_weights=None, constr_likelihood = 1E-6, verbatim = True, seed = 42):
    ...
    def calc_posterior_constrained(self, XS, compute_mode = False, num_samples = 1000, save_samples = 10, percentiles = [10, 50, 90], algorithm = 'minimax_tilting', resample = False):
        ...
        dim = XS.shape[0]
        U_sim = np.matrix(self.rng.multivariate_normal(np.zeros(dim), np.eye(dim), size = num_samples)).T
        ...
        randints = self.rng.choice(num_samples, save_samples)
        ...
        perc = np.percentile(np.squeeze(np.asarray(fs_sim)), np.squeeze(np.asarray(percentiles)), axis = 1)
    ...
    def optimize(self, include_constraint = True, fix_likelihood = False, bounds = True, likelihood_bound = (1e-6, 1e6), var_bound = (1e-6, 1e6), lengthscale_bounds = [(1e-6, 1e6)], conditional = False, pc_alg = 'minimax_tilting', n = 100):
        """
        ...
        likelihood_bound -> two-sided bound, array 
        var_bound -> two-sided bound, array 
        lengthscale_bounds -> array of arrays, one for each length scale of the kernel 
        ...
        if has_constr and include_constraint:
            ...
            optimized = self._optimize_constrained(fix_likelihood = fix_likelihood, likelihood_bound = likelihood_bound, var_bound = var_bound, lengthscale_bounds = lengthscale_bounds, conditional = conditional, algorithm = pc_alg, n = n, bounds = bounds)
            return optimized
            ...
            optimized = self._optimize_unconstrained(method = 'ML', fix_likelihood = fix_likelihood, likelihood_bound = likelihood_bound, var_bound = var_bound, lengthscale_bounds = lengthscale_bounds, bounds = bounds)
            return optimized
    ...
    def find_XV_subop(self, p_target, Omega = None, batch_size = None, bounds = None, i_range = None, nu = None, max_iterations = 200, moment_approximation = False, num_samples = 1000, min_prob_unconstr_xv = None, sampling_alg = 'minimax_tilting', moment_alg = 'correlation-free', opt_method = 'differential_evolution', print_intermediate = True):
        ...
        for j in range(max_iterations):
            ...
            if pc_min >= p_target:
                ...
                if (j+1 >= 25) and (np.mean(pc_mins) < 1e-2):
                ...
                if j+1 == max_iterations:
                    if self.verbatim: 
                        print('DONE - Found {} points. Min. constraint prob = {}. Total time spent = {}'.format(j+1, pc_min, formattime(time.time() - t0)))
                    else:
                        print(f'Min. constraint prob = {pc_min}. Minimum constraint probability requirement not reached.')
                    break
    ...
    def _optimize_unconstrained(self, method = 'ML', fix_likelihood = False, likelihood_bound = (1e-6, 1e6), var_bound = (1e-6, 1e6), lengthscale_bounds = [(1e-6, 1e6)], bounds = True):
        """
        ...
        # Run optimization
        if method == 'ML':
            res = self._optimize_unconstrained_ML(fix_likelihood, likelihood_bound = likelihood_bound, var_bound = var_bound, lengthscale_bounds = lengthscale_bounds, bounds = bounds)
        elif method == 'CV':
            print('TODO...')
        ...
        if self.verbatim:
            if res.success:
                ...
                print('WARNING -- NO CONVERGENCE IN OPTIMIZATION')

        return res.success
    
    
    def _optimize_constrained(self, fix_likelihood = False, opt_method = 'shgo', likelihood_bound = (1e-6, 1e6), var_bound = (1e-6, 1e6), lengthscale_bounds = [(1e-6, 1e6)], conditional = False, algorithm = 'minimax_tilting', n = 10E3, opt_args = {}, bounds = True):
        """
        Optimize hyperparameters of unconstrained GP
        ...
        if not bounds:
            ...
            if fix_likelihood:
                bounds = [(var_bound[0], var_bound[1])]
                bounds = bounds + [(lengthscale_bounds[i][0], lengthscale_bounds[i][1]) for i in range(len(theta)-1)]

            else:
                bounds = [(likelihood_bound[0], likelihood_bound[1]), (var_bound[0], var_bound[1])]
                bounds = bounds + [(lengthscale_bounds[i][0], lengthscale_bounds[i][1]) for i in range(len(theta)-2)]
    ...
    def _argmin_pc_subop(self, i, nu, bounds, opt_method = 'differential_evolution', moment_approximation = False, sampling_alg = 'minimax_tilting', moment_alg = 'correlation-free', verbatim = False, num_samples = 1000, return_res = False):
        ...
        if self._no_const():
            ...
            if moment_approximation:
                ...
                def optfun(x, *args):
                    if np.any(np.isnan(x)) or np.any(np.isinf(x)): return np.inf
        ...
        if opt_method == 'differential_evolution':
            t = time.time()
            res = optimize.differential_evolution(optfun, bounds = bounds, args = args, maxiter=1000)
            tf = time.time() - t
    ...
    def _optimize_unconstrained_ML(self, fix_likelihood = False, likelihood_bound = (1e-6, 1e6), var_bound = (1e-6, 1e6), lengthscale_bounds = [(1e-6, 1e6)], bounds = True):
        """
        ...
        *** Need to run this if one of the following arrays are changed : ***
        ...
        if self.K_w is None:
            ...
            if self.noise_weights is not None:
                if self.X_training.shape[0] != self.noise_weights.shape[0]:
                    raise ValueError("X_training must have the same length as noise_weights " + "({:d}!={:d})".format(self.X_training.shape[0], self.noise_weights.shape[0]))
                noise_var = np.diag(self.noise_weights)
            else:
                noise_var = np.identity(n)

            self.K_w = np.matrix(self.kernel.K(self.X_training, self.X_training) + self.likelihood*noise_var) 
